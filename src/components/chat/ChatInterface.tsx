...
import { addReaction, deleteMessage, getConversations, getMessages, removeReaction } from "@/services/chat/chatService";
...
export function ChatInterface() {
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      try {
        const fetchedMessages = await getMessages(selectedConversationId);
  ...
  const handleReactToMessage = async (messageId: string, emoji: string) => {
    try {
      ...
      if (existingReactionIndex >= 0) {
        ...
        success = await removeReaction(messageId, currentUserId, emoji);
        updatedReactions = message.reactions.filter((_, index) => index !== existingReactionIndex);
      } else {
        // Add new reaction
        success = await addReaction(messageId, reaction);
