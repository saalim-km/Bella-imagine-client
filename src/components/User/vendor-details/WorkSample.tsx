    ...
    <Dialog> 
      <div className="mb-8"> 
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-4">
          {workSample.media.map((media, index) => (
            <DialogTrigger asChild key={media.url + index} onClick={() => handleMediaClick(index)}>
              <div className="rounded-md overflow-hidden cursor-pointer relative group">
      ...
      <DialogContent className="max-w-3xl lg:max-w-5xl max-h-[90vh] flex flex-col">
         ...
         <DialogHeader className="flex-shrink-0">
            ...
            {workSample.description && <DialogDescription>{workSample.description}</DialogDescription>}
         </DialogHeader>
         ...
         <div className="flex-grow min-h-0"> 
            ...
            >
               <CarouselContent className="flex-grow items-center"> 
                  {workSample.media.map((media, index) => (
                     <CarouselItem key={media.url + '-dialog-' + index} className="flex items-center justify-center">
                        <div className="w-full max-h-[65vh] flex items-center justify-center">
                         {media.type === "image" ? (
                           <img
                             src={media.url}
                             alt={`${workSample.title} - image ${index + 1}`}
                             className="object-contain w-auto h-auto max-w-full max-h-full rounded-md" // Use object-contain
                           />
                         ) : (
