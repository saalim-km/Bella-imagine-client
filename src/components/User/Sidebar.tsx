...
import { User, Calendar, CreditCard, Camera, MessageCircle, FileText } from "lucide-react"
...
export function Sidebar({ name, profileImage, activeTab, setActiveTab, role, hasCategory }: SidebarProps) {
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col items-center space-y-4 mb-8">
          <Avatar className="w-20 h-20 border-2 border-orange-200">
            <AvatarImage src={profileImage || "/placeholder.svg"} alt={name} className="object-cover" />
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600">{role === "vendor" ? "Photographer" : "Client"}</p>
          </div>
        </div>
        ...
        <nav className="space-y-1">
          ...
          {navItems.map((item) => (
            <Button
              ...
              } ${role === "vendor" && item.id !== "profile" && !hasCategory ? "opacity-50 pointer-events-none" : ""}`}
