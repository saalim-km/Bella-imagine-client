...
export const getAllVendors = async(data : IVendorsFilter) : Promise<PaginatedResponse<IVendorsResponse>> =>  {
    console.log(`in vendors listing hook : `,data);
    const response = await clientAxiosInstance.get('/client/vendors',{params : data});
    return response.data;
}

export const getAllClientCategories = async(): Promise<{success : boolean , data : Category[]}> => {
